...
describe('Bracket Match Linking (e2e)', () => {
      ...
      cy.get('[data-testid="match-card"]')
        ...
        .then((text) => {
          ...
          if (matchIdMatch) {
            ...
            // Click link button on first match card
            cy.get('[data-testid="link-btn"]').first().click();
  ...
  describe('View Details Functionality', () => {
    ...
    it('should open Dota match stats modal when clicking View Details', () => {
      ...
      // The DotaMatchStatsModal shows player stats
      cy.contains('Radiant', { timeout: 5000 }).should('be.visible');
