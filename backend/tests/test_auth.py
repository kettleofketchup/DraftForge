...
def reset_events_data(request):
    ...
    for event in events:
        ...
        # Clean up Discord records for this event
        DiscordEventLog.objects.filter(discord_event__event=event).delete()
        DiscordEvent.objects.filter(event=event).delete()

        if event.event_repeater_id is not None and event.name != "E2E Signup Event":
            # Delete events generated by repeaters (clean slate for generation tests)
            if event.tournament:
                event.tournament.delete()
            event.delete()
            deleted_generated += 1
        elif event.name == "E2E Signup Event":
