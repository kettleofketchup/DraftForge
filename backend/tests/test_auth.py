...
def generate_events(request):
    """
    TEST ONLY: Trigger event generation synchronously.

    Runs the same logic as the generate_upcoming_events Celery task, but
    synchronously so Playwright can create a repeater, trigger generation,
    and verify the event was created without waiting for the beat schedule.

    Returns:
        200: {"generated": <count>, "message": <task result string>}
        404: Not in test mode
    """
    if not isTestEnvironment(request):
        return Response({"detail": "Not Found"}, status=status.HTTP_404_NOT_FOUND)
...
def reset_events_data(request):
    ...
    # Reset the seeded E2E Signup Event to signups_open (don't delete signups or events)
    reset_count = 0
    deleted_generated = 0
    try:
        signup_event = Event.objects.get(
            organization__name=EVENTS_ORG_NAME, name="E2E Signup Event"
        )
        signup_event.state = EventState.SIGNUPS_OPEN
        if signup_event.tournament:
            old_tournament = signup_event.tournament
            signup_event.tournament = None
            signup_event.save()
            old_tournament.delete()
        else:
            signup_event.save()
        EventSignup.objects.filter(event=signup_event).delete()
        invalidate_obj(signup_event)
        reset_count += 1
    except Event.DoesNotExist:
        pass

    # Delete repeater-generated events (clean slate for generation tests)
    for event in Event.objects.filter(
        organization__name=EVENTS_ORG_NAME,
        event_repeater__isnull=False,
    ).exclude(name="E2E Signup Event"):
        DiscordEventLog.objects.filter(discord_event__event=event).delete()
        DiscordEvent.objects.filter(event=event).delete()
        if event.tournament:
