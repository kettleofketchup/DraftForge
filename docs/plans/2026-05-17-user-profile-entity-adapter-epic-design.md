...
16. **Carryover / merge helpers must split property-backed fields from concrete column fields.** Any `for field in (...)` + `model.save(update_fields=changed)` loop that mixes a `@property` (T1: `nickname`, `avatar`; T2: presumably `positions`; T3: org-scoped fields) with real DB columns raises `ValueError: 'The following fields do not exist in this model... <field>'` — Django validates `update_fields` against `_meta.fields`, and the property isn't one. Pattern fix: two tuples (`_CARRYOVER_PROPERTY_FIELDS`, `_CARRYOVER_COLUMN_FIELDS`); only column fields go into `update_fields`. Property setters persist independently. Example fix: `backend/app/discord_accounts.py` post-PR #250 rebase. T2 must audit any merge/migration helper that touches new property-backed fields; T3 same for org-scoped properties.

17. **Snapshot carryover values BEFORE the related-objects-repoint loop.** Merge helpers that walk `drop._meta.related_objects` will hit the new model's reverse-O2O (e.g. `BaseUserProfile.user`), try to repoint it to `keep`, fail with `IntegrityError` (uniqueness), and delete drop's child row. Any post-loop read of `drop.<property>` then returns `None` because the underlying child is gone — and the carryover silently no-ops. Capture the values into a dict before the loop, read from the snapshot afterward. Example: `discord_accounts.py:carryover_snapshot`. Applies any time T2/T3 add another property-backed CustomUser field that has a merge/reclaim flow above it.

18. **Dialog modals MUST use `<ScrollArea>` inside `DialogContent`, not raw `overflow-y-auto`.** The contract: `<DialogContent className="flex max-h-[90vh] flex-col overflow-hidden ...">` then `<ScrollArea className="-mx-6 min-h-0 flex-1 px-6">` wrapping the body. Without `overflow-hidden` on DialogContent, Radix Viewport never enables the themed scrollbar (visual drift between OS scrollbar and dialog theme). Reference: `docs/theming-guide/ai/references/scrollbars-dialogs.md`. T1's EditProfileModal lands this; T2/T3 modals (game/org tabs) inherit.

19. **New backend tests under `backend/<app>/tests/` are NOT auto-discovered by CI.** `backend/pytest.ini` sets `testpaths = tests`, which only collects `backend/tests/`. `playwright.yml`'s backend job has an explicit module list (around line 181). Any new T2/T3 app test directory (`user/tests`, `org/tests`, etc.) must be added to that list, otherwise the tests pass only when run manually. Example fix landed in T1 added `user.tests` to the workflow's `python manage.py test` invocation.

20. **Frontend Vitest is now wired into CI.** T1 added `"test": "vitest"` to `frontend/package.json` and a `Run frontend Vitest` step to `playwright.yml`'s backend job. New `*.test.ts` files under `frontend/app/` are now auto-discovered. T2/T3 must add per-store / per-hook Vitest coverage and can rely on CI catching regressions.

21. **Reverse-O2O cache pollution in merge/repoint loops.** Any helper that walks `model._meta.related_objects` and does `setattr(reverse_obj, fk_name, keep)` triggers Django's bidirectional O2O descriptor — it also sets `keep.<reverse_accessor> = reverse_obj` in keep's in-memory cache. If the subsequent `save()` raises and the code calls `reverse_obj.delete()`, keep's cache still holds the dead (pk=None) reference. Subsequent reads through that reverse accessor (e.g. `keep.<property>` proxying through `keep.base_profile.<field>`) read stale state, and writes raise `ValueError: Cannot force an update in save() with no primary key.` Fix: call `keep.refresh_from_db()` AFTER the loop completes to purge stale reverse caches. Example: `backend/app/discord_accounts.py:merge_discord_accounts`. T2 and T3 merge/reclaim helpers must do the same — the trap fires once any property-backed model has a reverse-O2O off CustomUser.

22. **Test container is pre-built from main — new dev deps need an image rebuild.** `playwright.yml` pulls `ghcr.io/.../backend-dev:latest` (built from main). Any new `[tool.poetry.group.dev.dependencies]` entry won't be in the image until `build-docker-images.yml` republishes. Guard test modules that import the new dep: `try: from <new_dep> import X / except ImportError: raise unittest.SkipTest(...)`. T1 did this for `django-test-migrations` in `backend/user/tests/test_migration.py`. T2/T3 must repeat the pattern for any new dev dep they introduce, OR coordinate an image rebuild PR before merging.

23. **Steam ID 32-bit / 64-bit sync is an invariant — tests that set one and expect the other to stay None are broken.** T1.5 resurrected `CustomUser.save()`'s steam-id sync (the two fields are the SAME Steam identity in different encodings — `steamid - STEAM_ID_64_BASE == steam_account_id`). After T1, any test that does `user.steamid = X; user.save()` will see `steam_account_id` auto-derived. Tests asserting "modal handler didn't modify steam_account_id" must compare against the post-save baseline, not `None`. Example fix: `backend/events/tests/test_signup_interactions.py` `test_dota_modal_writes_rank_status_and_returns_needs_rank_details`. T2/T3 tests inheriting `EventTestCase` (or any test that calls `user.save()` after setting one of the two fields) will hit this.

24. **Live-Redis cacheops behavioral tests are deferred to a follow-up.** T1's `test_cacheops_integration.py` is shipped but skipped — the warm/PATCH/re-fetch pattern doesn't produce predictable eviction under `TransactionTestCase` + Django autocommit + cacheops `keep_fresh=True`. The static grep guardrail in `test_cacheops.py` still validates that every nickname/avatar-shipping `@cached_as` block lists `BaseUserProfile` as a dep, which is the load-bearing invariant. Re-enable behavioral verification once the `keep_fresh` vs eviction timing is understood — likely needs `cache.clear()` between warm/re-fetch, or dropping `keep_fresh=True` on those blocks. T2/T3 cacheops verification can use the same grep-guardrail pattern + a deferred behavioral test.
